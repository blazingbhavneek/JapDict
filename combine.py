...
def count_japanese_chars(text: str) -> int:
    ...
    for char in text:
        ...
        if (
            ...
            or kanji_ext_range1[0] <= code <= kanji_ext_range1[1]
            or kanji_ext_range2[0] <= code <= kanji_ext_range2[1]
        ):
            count += 1
...
def combine_markdown_files(folder_path: str, output_file: str, min_japanese: int = 3):
    ...
    try:
        ...
        print(
            f"Successfully created '{output_file}' with {len(combined_text)} characters."
        )
        return True
